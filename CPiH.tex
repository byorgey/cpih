...
small lists as data structures.  That is, having two lists with $10^5$
elements each is probably fine; having $10^5$ lists with two elements
each is probably going to be too slow.
...
as \h{Data.Map.Strict} or \h{Data.IntMap}. In my experience it doesn't
make a whole lot of difference.  I tend to just stick to \h{Data.Map};
I don't know of any problems where using a different map variant makes
a significant difference.

It is worth having a good working knowledge of standard \h{Map}
functions like XXX

% \h{empty}, \h{singleton}, \h{fromList}, \h{toList},
% \h{insert}, \h{delete}, \h{union}/\h{unions}, \h{intersection},
% \h{difference}, \h{map}, \h{filter}, \h{member}, \h{null}, and
% \h{size}.

There are many others which are occasionally useful and can be looked
up in the documentation when needed, such as (but not limited to) XXX
